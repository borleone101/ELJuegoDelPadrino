...
import { calcularChances, getModoGanadores, getSorteoTheme } from "./logica_juego.js";
...
function fmtMoneyRedondo(n) {
  const v = Number(n||0);
  if(v === Math.round(v)) return `Bs ${Math.round(v)}`;
...
const _urgencyShown = new Set();
const _winnerShown = new Set();
    ...
    if(cuposLibres<=t && !_urgencyShown.has(key)){
      ...
      Swal.fire({
        html:`<div style="text-align:center;padding:.5rem 0">
          <div style="font-size:3.5rem;line-height:1;margin-bottom:.6rem">${msg.emoji}</div>
          <div style="font-family:'Oswald',sans-serif;font-size:1.35rem;font-weight:700;color:#fff;margin-bottom:.4rem">${msg.title}</div>
          <div style="font-size:.88rem;color:#8a7a62;margin-bottom:.9rem;line-height:1.5">${msg.body}</div>
          <div style="background:rgba(139,26,26,.12);border:1px solid rgba(139,26,26,.28);border-radius:9px;padding:.6rem 1rem;margin-bottom:.75rem;display:inline-block">
          ...
          <div style="display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:.78rem;color:#8a7a62">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#ef4444;animation:urgPulse 1s ease infinite"></span>
            <strong style="color:${cuposLibres<=3?"#ef4444":"#f87171"}">${cuposLibres} cupo${cuposLibres!==1?"s":""} disponible${cuposLibres!==1?"s":""}</strong>
        ...
        timer:18000,timerProgressBar:true,
        didOpen:()=>{
          if(!document.getElementById('urg-style')){
            const st=document.createElement('style');st.id='urg-style';
            st.textContent=`@keyframes urgPulse{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.5);opacity:.7}}`;
        ...
        if(r.isConfirmed){
          loadSection("sorteos");
          setTimeout(()=>window.abrirPanelCompra(round.id,gameName,round.numero,precio,round.cupos||0,round.game?.capacidad_max||25),300);
        }
      });
      break;
    }
...
function showWinnerAlert(roundId, gameNombre, numRonda) {
  ...
  Swal.fire({
    html:`<div style="text-align:center;padding:.5rem 0">
      <div style="font-size:3rem;margin-bottom:.5rem">${msg.emoji}</div>
      <div style="font-family:'Oswald',sans-serif;font-size:1.3rem;font-weight:700;color:#fff;margin-bottom:.35rem">${msg.title}</div>
      <div style="font-size:.85rem;color:#8a7a62;margin-bottom:.85rem;line-height:1.5">${msg.body}</div>
      <div style="background:rgba(139,26,26,.12);border:1px solid rgba(212,160,23,.3);border-radius:9px;padding:.6rem 1rem;display:inline-block">
    ...
    showCancelButton:true,confirmButtonText:`${msg.emoji} Ver ganadores`,cancelButtonText:"Después",
    confirmButtonColor:'#8b1a1a',cancelButtonColor:'#221c14',background:'#131009',color:'#e6dcc8',
    timer:20000,timerProgressBar:true,
  }).then(r=>{
    if(r.isConfirmed){loadSection("historial");setTimeout(()=>window.modalVerGanadores(roundId),400);}
  });
}

function toastUrgencia(cupos, roundId, gameNombre, numRonda, precio, cuposActuales, capacidadMax) {
  const thresholds=[10,5,3,1];
  for(const t of thresholds){
    const key=`toast-${roundId}-${t}`;
    if(cupos<=t && !_urgencyShown.has(key)){
      _urgencyShown.add(key);
      const color=cupos<=3?'#ef4444':'#f87171';
...
function setupRealtime() {
    ...
    .on("postgres_changes",{event:"*",schema:"public",table:"rounds"}, async(payload)=>{
      ...
      if(payload.eventType==="UPDATE"){
        if(payload.old?.estado==="abierta"&&payload.new?.estado==="sorteada"){
          let gName=payload.new?.game_nombre||"Sorteo";
          ...
          showWinnerAlert(payload.new.id,gName,payload.new?.numero||"—");
  ...
  supabase.channel("participations-urgency")
    .on("postgres_changes",{event:"INSERT",schema:"public",table:"participations"}, async(payload)=>{
      if(!puedeParticipar()) return;
      const roundId=payload.new?.round_id; if(!roundId) return;
      const{data:round}=await supabase.from("rounds").select("id,numero,estado,game_id").eq("id",roundId).eq("estado","abierta").single();
      if(!round) return;
      let gameData=null;
      if(round.game_id){const{data:gd}=await supabase.from("games").select("nombre,precio_boleto,capacidad_max").eq("id",round.game_id).single();gameData=gd;}
      const capacidadMax=gameData?.capacidad_max||25;
      const{count}=await supabase.from("participations").select("id",{count:"exact",head:true}).eq("round_id",roundId);
      const cuposOcupados=count||0;
      const cuposLibres=Math.max(0,capacidadMax-cuposOcupados);
      const roundEnriquecida={...round,cupos:cuposOcupados,game:gameData};
      const{count:yaCuento}=await supabase.from("participations").select("id",{count:"exact",head:true}).eq("round_id",roundId).eq("user_id",MY_USER_ID);
      const yaParticiping=(yaCuento||0)>0;
      if(!yaParticiping&&cuposLibres>0&&cuposLibres<=10){
        toastUrgencia(cuposLibres,roundId,gameData?.nombre||"Sorteo",round.numero,gameData?.precio_boleto||0,cuposOcupados,capacidadMax);
        const umbralesModal=[5,3,1];
        if(umbralesModal.includes(cuposLibres)) setTimeout(()=>showUrgencyAlert(roundEnriquecida,cuposLibres),800);
      }
    }).subscribe();
...
async function _checkRoundUrgency(roundData) {
  ...
  const capacidadMax=roundData.game?.capacidad_max||25;
  const{count}=await supabase.from("participations").select("id",{count:"exact",head:true}).eq("round_id",roundData.id);
  const cuposLibres=Math.max(0,capacidadMax-(count||0));
  if(cuposLibres<=10&&cuposLibres>0){
    const{count:yaCuento}=await supabase.from("participations").select("id",{count:"exact",head:true}).eq("round_id",roundData.id).eq("user_id",MY_USER_ID);
    if((yaCuento||0)===0) toastUrgencia(cuposLibres,roundData.id,roundData.game?.nombre||"Sorteo",roundData.numero,roundData.game?.precio_boleto||0,count||0,capacidadMax);
  }
}
...
window.modalSubirQR = async (esAct=false) => {
  const{value:v}=await Swal.fire({
    ...
    html:`<div style="text-align:left">
      <div style="background:rgba(139,26,26,.06);border:1px solid rgba(139,26,26,.2);border-radius:10px;padding:.8rem 1rem;margin-bottom:1rem;font-size:.82rem;color:var(--muted)">
      ...
      <div class="field" style="margin-bottom:1rem">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin-top:.2rem">
          ${METODOS_QR.map(m=>`<label class="metodo-card" data-val="${m.value}" style="border-color:${m.value===(qrState.metodo||'')?'var(--red2)':'var(--border)'};background:${m.value===(qrState.metodo||'')?'rgba(139,26,26,.14)':'var(--ink3)'}"><input type="radio" name="qrMetodo" value="${m.value}" ${m.value===(qrState.metodo||'')?'checked':''} style="margin-top:.15rem;accent-color:var(--red2)"><div><div style="font-size:.85rem;font-weight:600;color:#fff">${m.label}</div><div style="font-size:.7rem;color:var(--muted)">${m.desc}</div></div></label>`).join("")}
        </div>
      </div>
      <div class="field">
      ...
      <img id="qrPreviewImg" style="display:none;max-height:160px;width:100%;object-fit:contain;margin-top:.6rem;border-radius:8px;border:1px solid rgba(139,26,26,.2)">
    </div>`,
    showCancelButton:true,confirmButtonText:`<i class='bi bi-upload'></i> ${esAct?'Actualizar':'Subir QR'}`,
    cancelButtonText:"Cancelar",width:500,...swal$,
    didOpen:()=>{
      document.querySelectorAll(".metodo-card").forEach(c=>{
        c.addEventListener("click",()=>{
          document.querySelectorAll(".metodo-card").forEach(x=>{x.style.borderColor="var(--border)";x.style.background="var(--ink3)";});
          c.style.borderColor="var(--red2)";c.style.background="rgba(139,26,26,.14)";
        });
      });
...
window.modalVerMiQR=()=>{
  ...
  Swal.fire({
    ...
    html:`<img src="${qrState.url}" style="width:100%;max-height:280px;object-fit:contain;border-radius:10px;border:1px solid rgba(139,26,26,.22);margin-bottom:.8rem">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem;text-align:left">
      ...
      <div style="background:var(--ink3);border:1px solid var(--border);border-radius:8px;padding:.6rem"><div style="font-size:.68rem;color:var(--muted);margin-bottom:.2rem">ESTADO</div><div>${qrState.verificado?'<span style="color:#22c55e">✅ Verificado</span>':'<span style="color:#f87171">⏳ En revisión</span>'}</div></div>
    </div>`,
...
async function estadoBoletoGratisEnRonda(roundId) {
  ...
  return{yoUse:(miUso?.length||0)>0||miGratis,otrosConGratis:Math.max(0,(totalGratisEnRonda||0)-(miGratis?1:0)),totalGratisEnRonda:totalGratisEnRonda||0};
}

/* ═══════════════════════════════════════
   SORTEOS ACTIVOS
   - 3 columnas en desktop (sorteos-grid-3)
   - Sorteos llenos: siguen visibles, mueven al final, muestran "LLENO"
...
async function loadSorteos(silencioso=false) {
  ...
  if(bannerEl){
    ...
    if(boletosGratis>0&&puedeParticipar()){
      ...
      bfb.innerHTML=`<i class="bi bi-gift-fill bfb-icon"></i><div><div class="bfb-title">Tienes ${boletosGratis} boleto${boletosGratis>1?"s":""} gratis</div><div class="bfb-sub">¡Úsalos en el sorteo que quieras!</div></div>`;
  ...
  const roundsData=await Promise.all(rounds.map(async r=>{
    const capacidadMax=gamesMap[r.game_id]?.capacidad_max||25;
    ...
    return{...r,cupos,capacidadMax,game:gamesMap[r.game_id],misBoletos,miPago:myPay,yoUseGratis,otrosConGratis};
  }));

  // Ordenar: participando primero → libres → llenos al final
  const conMi=roundsData.filter(r=>r.misBoletos>0&&r.cupos<r.capacidadMax);
  const libres=roundsData.filter(r=>r.misBoletos===0&&r.cupos<r.capacidadMax).sort((a,b)=>b.cupos-a.cupos);
  const llenos=roundsData.filter(r=>r.cupos>=r.capacidadMax);
  const ordenados=[...conMi,...libres,...llenos];

  _inyectarCSSSorteos();

  container.innerHTML=`<div id="sorteosCards" class="sorteos-grid-3"></div>`;
  const cardsEl=getEl("sorteosCards");
  cardsEl.innerHTML=ordenados.map(r=>renderCardV2(r)).join("");
}

/* ── Render card nueva (limpia, 3 cols) ── */
function renderCardV2(r) {
  ...
  const cuposLibres=Math.max(0,capacidadMax-r.cupos);
  ...
  const precio=r.game?.precio_boleto||0;

  // Imagen / cabecera
  const urgencyRibbon=!estoyDentro&&!lleno&&cuposLibres<=5
    ?`<div class="sc-ribbon">${cuposLibres===1?"¡ÚLTIMO!":"¡QUEDAN "+cuposLibres+"!"}</div>`:
    lleno?`<div class="sc-ribbon-lleno">🔒 LLENO</div>`:"";

  const participatingBadge=estoyDentro?`<div class="sc-participating"><i class="bi bi-person-fill-check"></i> Participando</div>`:"";

  const imgHeader=imgUrl
    ?`<div class="sc-img">
        <img src="${imgUrl}" alt="${r.game?.nombre||""}" class="sc-img-real" loading="lazy" decoding="async"
          onerror="this.closest('.sc-img').classList.add('sc-img-fallback');this.style.display='none'">
        <div class="sc-img-overlay"></div>
        <div class="sc-badges">
          <span class="sc-badge ${modo===1?'sc-badge-winner1':'sc-badge-winner3'}">${modo===1?"🥇 1 ganador":"🏅 3 ganadores"}</span>
          ${precio===0?`<span class="sc-badge sc-badge-free">GRATIS</span>`:""}
        </div>
        ${participatingBadge}
        ${urgencyRibbon}
      </div>`
    :`<div class="sc-img" style="background:${theme.gradient}">
        <div class="sc-img-icon">${theme.icon}</div>
        <div class="sc-badges">
          <span class="sc-badge ${modo===1?'sc-badge-winner1':'sc-badge-winner3'}">${modo===1?"🥇 1 ganador":"🏅 3 ganadores"}</span>
          ${precio===0?`<span class="sc-badge sc-badge-free">GRATIS</span>`:""}
        </div>
  ...
  // Cuerpo — solo datos esenciales
  const body=`<div class="sc-body">
    <div class="sc-meta">
      <div class="sc-nombre">${r.game?.nombre??"—"}</div>
      ${precio>0
        ?`<div class="sc-precio">${fmtMoney(precio)}<span style="font-size:.7rem;color:var(--muted);font-weight:400">/boleto</span></div>`
        :`<div class="sc-precio-free">¡GRATIS!</div>`}
    </div>
    <div class="sc-prog-wrap">
      <div class="sc-prog-label">
        <span style="color:var(--muted)">Jugadores</span>
        <span style="${lleno?"color:#22c55e;font-weight:700":""}">${r.cupos}/${capacidadMax}${lleno?" 🔒":""}</span>
      </div>
      <div class="sc-prog-bg">
        <div class="sc-prog-fill" style="width:${Math.min(pct,100)}%;background:${lleno?"linear-gradient(90deg,#166534,#22c55e)":`linear-gradient(90deg,${theme.accent}88,${theme.accent})`}"></div>
      </div>
    </div>
    ${boletosGratis>0&&!r.yoUseGratis&&!lleno&&puedeParticipar()
      ?`<div style="font-size:.73rem;color:#4ade80;display:flex;align-items:center;gap:.3rem"><i class="bi bi-gift-fill"></i> Tienes boleto gratis disponible</div>`:""
    }
  </div>`;
  ...
  let btnHtml="";
  if(lleno){
    // Sorteo lleno: visible pero con botón que muestra alert de "en proceso"
    btnHtml=`<button class="btn btn-ghost btn-sm" style="opacity:.7" onclick="alertSorteoLleno()"><i class="bi bi-hourglass-split"></i> En proceso</button>`;
  } else if(!puedeParticipar()){
    btnHtml=!qrState.subido
      ?`<button class="btn btn-red btn-md" onclick="modalSubirQR()"><i class="bi bi-qr-code-scan"></i> Subir QR</button>`
      :`<button class="btn btn-ghost btn-md" disabled><i class="bi bi-hourglass-split"></i> Verificando QR</button>`;
  } else if(tieneCompPend){
    btnHtml=`<span class="bdg bdg-p"><i class="bi bi-hourglass-split"></i> En revisión</span>`;
  } else if(tieneCompAprobado&&estoyDentro){
    btnHtml=`<span class="bdg bdg-ok" style="font-size:.65rem"><i class="bi bi-check-circle-fill"></i> ${r.misBoletos} boleto${r.misBoletos>1?"s":""}</span>
      <button class="btn btn-ghost btn-sm" onclick="abrirPanelCompra('${r.id}','${(r.game?.nombre||"").replace(/'/g,"\\'")}','${r.numero}',${precio},${r.cupos},${capacidadMax})"><i class="bi bi-plus-circle"></i> Más</button>`;
  } else {
    btnHtml=`<button class="btn btn-red btn-md" onclick="abrirPanelCompra('${r.id}','${(r.game?.nombre||"").replace(/'/g,"\\'")}','${r.numero}',${precio},${r.cupos},${capacidadMax})"><i class="bi bi-ticket-perforated-fill"></i> ${precio===0?"Participar gratis":"Participar"}</button>`;
  }

  const foot=`<div class="sc-foot">
    ${estoyDentro&&!lleno?`<div class="sc-mis-boletos"><i class="bi bi-ticket-perforated-fill"></i> <strong>${r.misBoletos}</strong> boleto${r.misBoletos>1?"s":""}</div>`:`<div></div>`}
    <div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap">${btnHtml}</div>
  </div>`;

  const clases=`sorteo-card-v2${estoyDentro?" sc-participando":""}${lleno?" sc-lleno":""}`;
  const borderStyle=!estoyDentro&&!lleno&&cuposLibres<=3?"border-color:rgba(239,68,68,.4);":"";

  return `<div class="${clases}" style="${borderStyle}">${imgHeader}${body}${foot}</div>`;
}

/* Alert sorteo lleno */
window.alertSorteoLleno = () => {
  Swal.fire({
    title:"🔒 Sorteo lleno",
    html:`<div style="text-align:center;padding:.5rem 0">
      <div style="font-size:2.5rem;margin-bottom:.5rem">⏳</div>
      <div style="color:var(--cream);font-size:.95rem;margin-bottom:.5rem">Este sorteo ya está completo y el sorteo está en proceso.</div>
      <div style="font-size:.82rem;color:var(--muted)">Pronto se anunciarán los ganadores. ¡Mantente atento!</div>
    </div>`,
...
function _inyectarCSSSorteos() {
  if(document.getElementById('sorteo-anim-css')) return;
  const s=document.createElement('style');s.id='sorteo-anim-css';
  s.textContent=`
    @keyframes urgPulse2{0%,100%{opacity:1}50%{opacity:.5}}
    .sc-img-fallback{background:linear-gradient(135deg,#1c1407,#3d2b0f,#8b1a1a)!important}
  `;
  document.head.appendChild(s);
}

/* ═══════════════════════════════════════
   PANEL COMPRA — colores rojo/oscuro
═══════════════════════════════════════ */
window.abrirPanelCompra = async (roundId, gameNombre, numRonda, precioBoleto, cuposActuales, capacidadMax=25) => {
  ...
  <div class="compra-drawer-panel">
    <div class="compra-drawer-header" style="background:${theme.gradient};position:relative;overflow:hidden;">
      <div style="position:absolute;inset:0;background:linear-gradient(to right,rgba(0,0,0,.55),rgba(0,0,0,.2))"></div>
      <div style="position:relative;z-index:1">
        ...
        <div style="font-size:.75rem;color:rgba(255,255,255,.75);display:flex;align-items:center;gap:.5rem;margin-top:.18rem;flex-wrap:wrap">
          <span>Ronda #${numRonda}</span><span>·</span>
          <span>${cuposLibres} cupo${cuposLibres!==1?"s":""} libres</span><span>·</span>
          <span style="color:${theme.accent}">${modoLabel}</span>
        </div>
      </div>
      <div style="position:relative;z-index:1;display:flex;align-items:center;gap:.5rem">
    ...
    <div class="compra-drawer-body" id="compraBody">
      ...
      <div class="compra-step" id="compraStep1">
        ...
        <div class="compra-gratis-box">
          <div style="display:flex;align-items:center;gap:.75rem">
            ...
            <div><div style="font-family:'Oswald',sans-serif;font-size:.9rem;color:#22c55e">Tienes ${boletosGratis} boleto${boletosGratis>1?"s":""} gratis</div><div style="font-size:.75rem;color:var(--muted)">1 por sorteo</div></div>
          </div>
          <label class="compra-gratis-toggle"><input type="checkbox" id="drawerUsarGratis"><span class="gratis-toggle-label">Usar 1 gratis</span></label>
        </div>`:
        gratisStatus.yoUse?`<div style="background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.22);border-radius:8px;padding:.6rem .9rem;margin-bottom:.75rem;font-size:.82rem;color:#86efac;display:flex;align-items:center;gap:.5rem"><i class="bi bi-check-circle-fill"></i> Ya usaste tu boleto gratis aquí.</div>`:""}

        ${!gratisStatus.yoUse&&gratisStatus.otrosConGratis>0&&boletosGratis>0
          ?`<div class="compra-alert-warn"><i class="bi bi-lightning-charge-fill"></i>${gratisStatus.otrosConGratis} jugador${gratisStatus.otrosConGratis>1?"es":""} ya tienen boleto gratis aquí.</div>`:""
        }
        ...
        ${precioBoleto===0
          ?`<button class="compra-btn-next btn-solo-gratis" id="drawerBtnNext"><i class="bi bi-ticket-perforated-fill"></i> Confirmar participación gratis</button>`
      ...
      <div class="compra-step" id="compraStep2" style="display:none">
        <div class="compra-resumen-fijo">
          ...
          <button class="compra-back-btn" id="drawerBack" title="Volver"><i class="bi bi-arrow-left"></i></button>
        </div>
  ...
  let boletos=1,usarGratis=false,boletosSnap=1,usarGratisSnap=false,bapSnap=1;
  ...
  getEl("drawerBtnNext")?.addEventListener("click",()=>{
    ...
    const rt=getEl("drawerResumenTxt");const rm=getEl("drawerResumenMonto");
  ...
  getEl("drawerBack")?.addEventListener("click",()=>{
    ...
    const compInput=getEl("drawerComp");if(compInput) compInput.value="";
    ...
    r2.onload=ev=>{
      ...
      if(backBtn){
        ...
        if(resumen&&!resumen.querySelector(".compra-lock-badge")){
          const badge=document.createElement("span");badge.className="compra-lock-badge";
          badge.innerHTML='<i class="bi bi-lock-fill"></i> Fijo';resumen.appendChild(badge);
        }
      }
    };
    r2.readAsDataURL(f);
  ...
  async function enviarParticipacion(){
    const _boletos=(getEl("compraStep2")?.style.display!=="none")?boletosSnap:boletos;
    const _usarGratis=(getEl("compraStep2")?.style.display!=="none")?usarGratisSnap:usarGratis;
...
window.cerrarCompraDrawer=()=>{
  const d=document.getElementById("compraDrawer");if(!d) return;
...
window.modalVerGanadores = async (roundId) => {
  ...
  const[{data:game}]=await Promise.all([
    round?.game_id?supabase.from("games").select("nombre,capacidad_max").eq("id",round.game_id).single():{data:null},
  ]);
  ...
  const modo=getModoGanadores(game?.capacidad_max||25);
  const theme=getSorteoTheme(game?.nombre||"");

  let screen=document.getElementById("ganadoresScreen");
  if(screen) screen.remove();
  screen=document.createElement("div");
  screen.id="ganadoresScreen";screen.className="ganadores-screen";
  screen.innerHTML=`
  <div class="ganadores-bg" style="background:radial-gradient(ellipse at center,#1a0d05 0%,#0a0603 100%)">
    <div style="position:absolute;inset:0;background:${theme.gradient};opacity:.15;pointer-events:none"></div>
    ...
    <div class="ganadores-content">
      ...
      <div class="ganadores-titulo-wrap">
        ...
        <div style="margin-top:.5rem;display:inline-flex;align-items:center;gap:.4rem;font-family:'Oswald',sans-serif;font-size:.7rem;letter-spacing:.1em;color:${theme.accent};background:${theme.accent}18;border:1px solid ${theme.accent}44;border-radius:20px;padding:.18rem .7rem">
          ${theme.icon} ${modo===1?"1 GANADOR":"3 GANADORES"}
        </div>
      </div>
      <div class="ganadores-img-wrap">
        <img src="https://res.cloudinary.com/daxmlrngo/image/upload/v1772453389/9c927cfe-c983-4252-8945-a70b24c03eb1_rtrp4u.png" class="ganadores-img" alt="El Padrino" style="border-color:${theme.accent}88">
        <div class="ganadores-img-glow" style="background:radial-gradient(circle,${theme.accent}22,transparent 70%)"></div>
      </div>
      <div class="ganadores-podio">
        ${g2&&modo===3?`<div class="podio-card podio-2" style="animation-delay:.4s"><div class="podio-trofeo">🥈</div><div class="podio-puesto">2do Lugar</div><div class="podio-nombre">${g2}</div></div>`:""}
        ${g1?`<div class="podio-card podio-1" style="animation-delay:.1s;border-color:${theme.accent}66"><div class="podio-corona">👑</div><div class="podio-trofeo">🥇</div><div class="podio-puesto">1er Lugar</div><div class="podio-nombre">${g1}</div></div>`:""}
        ${g3&&modo===3?`<div class="podio-card podio-3" style="animation-delay:.6s"><div class="podio-trofeo">🥉</div><div class="podio-puesto">3er Lugar</div><div class="podio-nombre">${g3}</div></div>`:""}
      </div>
      ...
      <div class="ganadores-creditos-scroll" id="ganadoresCreditos">
        <div class="creditos-track" id="creditosTrack">
          ${participantes.map(p=>`<div class="credito-item"><span class="credito-username">${p.username}</span><span class="credito-info">${p.boletos} boleto${p.boletos>1?"s":""}${p.esGratis?` · <span style="color:#4ade80"><i class="bi bi-gift-fill"></i></span>`:""}</span></div>`).join("")}
  ...
  if(track){const h=track.scrollHeight;track.style.animation=`creditosScroll ${Math.max(8,h/30)}s 2s linear infinite`;}

  let ganadoresAudio=document.getElementById("ganadoresAudio");
  if(!ganadoresAudio){
    ganadoresAudio=document.createElement("audio");ganadoresAudio.id="ganadoresAudio";
    ...
    ganadoresAudio.loop=true;ganadoresAudio.volume=0.55;document.body.appendChild(ganadoresAudio);
  }
  ganadoresAudio.currentTime=0;ganadoresAudio.volume=0;
  const playPromise=ganadoresAudio.play();
  if(playPromise!==undefined){
    playPromise.catch(()=>{
      const closeBtn=screen.querySelector(".ganadores-close");
      if(closeBtn){
        const playBtn=document.createElement("button");playBtn.className="ganadores-play-btn";
        playBtn.innerHTML='<i class="bi bi-music-note-beamed"></i>';playBtn.title="Activar música";
        playBtn.onclick=()=>{
          ganadoresAudio.play();
  ...
  let vol=0;const fadeIn=setInterval(()=>{vol=Math.min(vol+0.04,0.55);ganadoresAudio.volume=vol;if(vol>=0.55)clearInterval(fadeIn);},100);
};

window.cerrarGanadoresScreen=()=>{
  const s=document.getElementById("ganadoresScreen");if(!s) return;
  const audio=document.getElementById("ganadoresAudio");
  if(audio&&!audio.paused){
    let v=audio.volume;const fadeOut=setInterval(()=>{v=Math.max(v-0.06,0);audio.volume=v;if(v<=0){clearInterval(fadeOut);audio.pause();audio.currentTime=0;}},80);
  }
...
async function loadHistorial() {
  ...
  const renderItem=p=>`<div class="list-item">
    ...
    <div class="li-right">${resBdg(p)}${p.roundEstado==="sorteada"?`<button class="btn btn-ghost btn-sm" style="margin-top:.3rem" onclick="modalVerGanadores('${p.round_id}')"><i class="bi bi-trophy-fill"></i> Ganadores</button>`:""}</div>
  </div>`;
...
async function loadReferidos() {
  ...
  const renderRef=r=>`<div class="list-item">
    ...
    <div class="li-body"><div class="li-title">${r.profiles?.username||"Usuario"}</div><div class="li-sub">${fmtDateShort(r.creado_at)}${r.boleto_otorgado?` · <span style="color:#4ade80"><i class="bi bi-gift-fill"></i> Boleto ganado</span>`:""}</div></div>
  ...
  <div class="panel"><div class="panel-head"><div class="panel-title"><i class="bi bi-share-fill"></i>Tu código de invitación</div></div><div class="panel-body">
    <div class="ref-code-box">
      ...
      <button class="btn btn-red btn-md" onclick="copiarCodigo('${codigoRef}')"><i class="bi bi-copy"></i> Copiar código</button>
    </div>
    <div style="background:rgba(139,26,26,.05);border:1px solid rgba(139,26,26,.15);border-radius:9px;padding:.75rem 1rem;font-size:.82rem;color:var(--muted)">
      ...
      <ul style="padding-left:1rem;line-height:1.8">
        ...
        <li>Cuando compre <strong style="color:var(--cream)">3 boletos pagados</strong>, recibes <strong style="color:#22c55e">1 boleto gratis</strong></li>
        <li>Solo 1 boleto gratis por sorteo</li>
      </ul>
    </div>
  </div></div>
...
/* ═══════════════════════════════════════
   RESPETO Y OFERTAS (Fidelidad)
   - Lenguaje simple
   - Grupos con nombres que la gente entiende
═══════════════════════════════════════ */
async function loadFidelidad() {
  ...
  // Grupos con lenguaje simple
  const grupos = [
    {
      label:"🚀 Primeros pasos", sub:"Empieza aquí",
      logros:[
        {nombre:"¡Bienvenido!",   desc:"Juega en tu primer sorteo",        icono:"bi-door-open-fill",  logrado:totalBoletos>=1,  prog:Math.min(totalBoletos,1),  max:1,   xp:10},
        {nombre:"Arrancando",     desc:"Juega en 5 sorteos",               icono:"bi-speedometer2",    logrado:totalBoletos>=5,  prog:Math.min(totalBoletos,5),  max:5,   xp:25},
        {nombre:"Primera victoria",desc:"Gana tu primer sorteo",           icono:"bi-star-fill",       logrado:totalGanadas>=1,  prog:Math.min(totalGanadas,1),  max:1,   xp:50},
      ]
    },
    {
      label:"📅 Constancia paga", sub:"Sigue jugando",
      logros:[
        ...
        {nombre:"Veterano",       desc:"Juega en 20 sorteos",              icono:"bi-calendar-check",  logrado:totalBoletos>=20, prog:Math.min(totalBoletos,20),  max:20,  xp:60},
        {nombre:"El Fiel",        desc:"Juega en 50 sorteos",              icono:"bi-patch-star-fill", logrado:totalBoletos>=50, prog:Math.min(totalBoletos,50),  max:50,  xp:100},
        {nombre:"Cien sorteos",   desc:"100 sorteos jugados",              icono:"bi-award-fill",      logrado:totalBoletos>=100,prog:Math.min(totalBoletos,100), max:100, xp:200},
      ]
    },
    {
      label:"💰 Cuánto apostaste", sub:"Metas de inversión",
      logros:[
    ...
    {
      label:"👥 Trae amigos", sub:"Más ganas juntos",
      logros:[
        {nombre:"Reclutador",     desc:"Trae 3 amigos que jueguen",        icono:"bi-people-fill",     logrado:refsActivos>=3,  prog:Math.min(refsActivos,3),   max:3,   xp:60},
        {nombre:"Red grande",     desc:"Trae 10 amigos que jueguen",       icono:"bi-diagram-3-fill",  logrado:refsActivos>=10, prog:Math.min(refsActivos,10),  max:10,  xp:120},
      ]
    },
    {
      label:"👑 Los mejores", sub:"Solo los más jugadores llegan",
      logros:[
        {nombre:"Triple ganador", desc:"Gana 3 sorteos",                   icono:"bi-trophy-fill",     logrado:totalGanadas>=3,  prog:Math.min(totalGanadas,3),  max:3,   xp:100},
        {nombre:"Gran ganador",   desc:"Gana Bs 100 en premios",           icono:"bi-cash-coin",       logrado:totalGanado>=100, prog:Math.min(totalGanado,100), max:100, xp:150},
        ...
        {nombre:"Leyenda",        desc:"200 sorteos jugados",              icono:"bi-fire",            logrado:totalBoletos>=200,prog:Math.min(totalBoletos,200), max:200, xp:500},
      ]
    }
  ];
  ...
  function renderGrupos() {
    return grupos.map(g=>{
      ...
      const logroItems=g.logros.map(l=>{
        ...
        return `<div class="lg-card ${l.logrado?"lg-done":""}">
          <div class="lg-icon ${l.logrado?"lg-icon-done":""}"><i class="bi ${l.icono}"></i></div>
          <div class="lg-body">
            <div class="lg-nombre">${l.nombre}</div>
            <div class="lg-desc">${l.desc}</div>
            <div class="lg-barra-wrap">
              <div class="lg-barra"><div class="lg-barra-fill ${l.logrado?"lg-barra-ok":""}" style="width:${pct}%"></div></div>
              <span class="lg-prog-txt">${l.prog}/${l.max}</span>
            </div>
          </div>
          <div class="lg-xp ${l.logrado?"lg-xp-done":"lg-xp-lock"}">
            ${l.logrado?`<i class="bi bi-check-circle-fill" style="font-size:.7rem"></i>`:`<i class="bi bi-lock-fill" style="font-size:.65rem"></i>`}${l.xp}
          </div>
        </div>`;
      }).join("");
      return `<div class="lg-grupo">
    ...
    <div class="panel-body nivel-body">
      <div class="nivel-row">
        ...
        <div class="nivel-info">
          ...
          <div class="nivel-badge ${nivel.clase}"><i class="bi bi-star-fill"></i> ${totalBoletos} sorteo${totalBoletos!==1?"s":""} jugado${totalBoletos!==1?"s":""}</div>
        </div>
      ...
      <div class="nivel-barra-bg"><div class="nivel-barra-fill" style="width:${proxNivel.pct}%"></div></div>
      <div class="nivel-prog-hint">Faltan ${proxNivel.requerido-proxNivel.progreso} sorteo${proxNivel.requerido-proxNivel.progreso!==1?"s":""} más</div>
      `:`<div class="nivel-max-msg"><i class="bi bi-crown-fill"></i> ¡Nivel máximo! 🏆</div>`}
    </div>
  </div>

  <div class="fid-grid">
    <div class="fid-cell"><i class="bi bi-ticket-perforated-fill fid-ico"></i><div class="fid-num">${totalBoletos}</div><div class="fid-lbl">Jugados</div></div>
    ...
    <div class="fid-cell"><i class="bi bi-people-fill fid-ico" style="color:#818cf8"></i><div class="fid-num">${refsActivos}</div><div class="fid-lbl">Amigos</div></div>
  ...
  ${bgsTotal>0?`<div class="boleto-gratis-banner">
    ...
    <div><div class="bfb-title">${bgsTotal} boleto${bgsTotal>1?"s":""} gratis disponible${bgsTotal>1?"s":""}</div><div class="bfb-sub">¡Úsalos en cualquier sorteo!</div></div>
    <button class="btn btn-green btn-sm" onclick="loadSection('sorteos')"><i class="bi bi-ticket-perforated-fill"></i> Usar</button>
  </div>`:""}

  <div class="panel">
    <div class="panel-head">
      <div class="panel-title"><i class="bi bi-stars"></i>Tus logros</div>
      <span class="lg-total-badge">${logradosTotal}/${totalLogros}</span>
    </div>
    <div class="panel-body" style="padding:.5rem .75rem .75rem">${renderGrupos()}</div>
  </div>`;
}
...
async function loadPerfil() {
  ...
  <div class="panel">
    <div class="panel-head"><div class="panel-title"><i class="bi bi-bar-chart-fill"></i>Mis números</div></div>
    <div class="panel-body">
      <div class="stats-grid">
        <div class="stat-box"><div class="stat-val">${totalBoletos}</div><div class="stat-lbl"><i class="bi bi-ticket-perforated"></i> Sorteos jugados</div></div>
        <div class="stat-box stat-win"><div class="stat-val">${totalGanados}</div><div class="stat-lbl"><i class="bi bi-trophy"></i> Veces que gané</div></div>
        <div class="stat-box"><div class="stat-val">${fmtMoney(totalGastado)}</div><div class="stat-lbl"><i class="bi bi-arrow-up-circle"></i> Lo que invertí</div></div>
        <div class="stat-box stat-gold"><div class="stat-val">${fmtMoney(totalGanado)}</div><div class="stat-lbl"><i class="bi bi-cash-stack"></i> Lo que gané</div></div>
        <div class="stat-box"><div class="stat-val">${refsActivos}</div><div class="stat-lbl"><i class="bi bi-people"></i> Amigos activos</div></div>
        <div class="stat-box stat-win"><div class="stat-val">${boletosGratis}</div><div class="stat-lbl"><i class="bi bi-gift"></i> Gratis disponibles</div></div>
      </div>
    </div>
  </div>
  <div class="panel">
    ...
    <div class="panel-body">${!qrState.subido
      ?`<div class="qr-empty-state"><div class="qes-icon"><i class="bi bi-qr-code-scan"></i></div><div class="qes-title">Sin QR de cobros</div><div class="qes-sub">Lo necesitas para participar y recibir tus premios.</div><button class="btn btn-red btn-md" style="margin-top:1rem" onclick="modalSubirQR()"><i class="bi bi-upload"></i> Subir QR</button></div>`
      :`<div class="qr-perfil-wrap"><img src="${qrState.url}" style="max-height:180px;max-width:100%;border-radius:10px;border:1px solid rgba(139,26,26,.22);object-fit:contain;cursor:pointer" onclick="modalVerMiQR()" loading="lazy" onerror="this.style.display='none'">
      <div class="qr-details-grid">
        <div class="qr-detail-box"><div class="qdb-label">Método</div><div class="qdb-val">${mlM[qrState.metodo]||qrState.metodo||"—"}</div></div>
        <div class="qr-detail-box"><div class="qdb-label">Estado</div><div class="qdb-val">${qrState.verificado?'<span class="bdg bdg-ok"><i class="bi bi-shield-check"></i> OK</span>':'<span class="bdg bdg-p"><i class="bi bi-hourglass-split"></i> En revisión</span>'}</div></div>
      </div></div>`
    }</div>
  </div>
  <div class="panel">
    <div class="panel-head"><div class="panel-title"><i class="bi bi-person-gear"></i>Mi cuenta</div></div>
    <div class="panel-body"><div class="account-rows">
      <div class="account-row"><div class="ar-label"><i class="bi bi-person"></i> Nombre</div><div class="ar-val">${prof?.username??"—"}</div></div>
      <div class="account-row"><div class="ar-label"><i class="bi bi-envelope"></i> Email</div><div class="ar-val">${user.email??"—"}</div></div>
      <div class="account-row"><div class="ar-label"><i class="bi bi-hash"></i> Código de invitación</div><div class="ar-val">${prof?.codigo_referido||"—"}${prof?.codigo_referido?`<button class="btn btn-ghost btn-sm" onclick="copiarCodigo('${prof.codigo_referido}')"><i class="bi bi-copy"></i></button>`:""}</div></div>
    </div>
