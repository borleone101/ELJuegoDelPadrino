  ...
  <div style="margin-bottom:.9rem">
    ...
    <div id="${prefix}PanelSubir">
      <div id="${prefix}DropArea"
        ...
        ondrop="handleImgDrop(event,'${prefix}')">
        <input type="file" id="${prefix}FileInput" accept="image/jpeg,image/png,image/webp" style="display:none" onchange="previewImgFile(this,'${prefix}')">
        <div id="${prefix}UploadPh" style="display:flex;flex-direction:column;align-items:center;gap:.3rem;color:#4a3c2a;font-size:.82rem;pointer-events:none">
        ...
        <img id="${prefix}FilePreview" style="display:none;width:100%;max-height:110px;object-fit:cover;border-radius:7px;position:relative;z-index:1">
      </div>
    </div>
    <div id="${prefix}PanelUrl" style="display:none">
      <input type="url" id="${prefix}UrlInput" placeholder="https://res.cloudinary.com/…"
        ...
        oninput="previewImgUrl(this.value,'${prefix}')">
      <div id="${prefix}UrlPreview" style="display:none;margin-top:.45rem;border-radius:7px;overflow:hidden;border:1px solid rgba(212,160,23,.2)">
    ...
    <div id="${prefix}ActualWrap" style="margin-top:.5rem">
      ...
      <div style="position:relative;border-radius:8px;overflow:hidden;border:1px solid rgba(212,160,23,.2);max-height:90px">
        ...
        <label style="position:absolute;bottom:.4rem;right:.5rem;display:flex;align-items:center;gap:.35rem;font-size:.72rem;color:#fff;cursor:pointer">
          <input type="checkbox" id="${prefix}QuitarImg" style="accent-color:#f87171;width:13px;height:13px">
  ...
  MC().innerHTML = `
    <div class="ph">
      ...
      <div style="display:flex;align-items:center;gap:.6rem">
        <span class="rt-dot"></span><span class="rt-label">En vivo</span>
  ...
  const { value:v } = await Swal.fire({
    ...
    html:`<div style="text-align:left">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:.85rem">
        <div class="field" style="margin-bottom:0"><label>Nombre *</label><input id="sNom" class="swal2-input" placeholder="ej. El Padrino…" style="margin:0;width:100%"></div>
        <div class="field" style="margin-bottom:0"><label>Descripción</label><input id="sDesc" class="swal2-input" placeholder="Breve descripción" style="margin:0;width:100%"></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;margin-bottom:.85rem">
        <div class="field" style="margin-bottom:0"><label>Precio boleto (Bs) *</label>
          <select id="sPrecio" class="swal2-input" style="margin:0;width:100%;background:#1b1610;border:1px solid rgba(139,26,26,.28);color:#e6dcc8;border-radius:8px;padding:.5rem .8rem">
            <option value="0">Gratis (0 Bs)</option><option value="5">Bs 5</option><option value="10" selected>Bs 10</option><option value="15">Bs 15</option>
          </select>
        </div>
        <div class="field" style="margin-bottom:0"><label>Capacidad máx. * <span style="color:var(--dim);font-size:.62rem;font-weight:400;text-transform:none;letter-spacing:0">≤25→1G · >25→3G</span></label>
      ...
      <div class="sdh-content">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div>
            ...
            <div style="font-size:.75rem;color:rgba(255,255,255,.7);margin-top:.15rem">${modo===1?"🥇 1 Ganador":"🏅 3 Ganadores"} · ${getCapacidad(game)} cupos${premioEst>0?` · <strong style="color:#4ade80">Bs ${premioEst}</strong>`:""}</div>
          </div>
          <div style="display:flex;align-items:center;gap:.5rem">
    ...
    <div class="sorteo-drawer-body">
      ...
      <div class="sdb-section">
        ...
        <div style="display:flex;flex-direction:column;gap:.65rem">
          <div><label class="sdb-label">Nombre *</label><input id="sdNombre" class="sdb-input" value="${(game.nombre||'').replace(/"/g,'&quot;')}" placeholder="Nombre del sorteo"></div>
          <div><label class="sdb-label">Descripción</label><input id="sdDesc" class="sdb-input" value="${(game.descripcion||'').replace(/"/g,'&quot;')}" placeholder="Descripción breve"></div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:.6rem">
            ...
            <div><label class="sdb-label">Capacidad máx.</label><input id="sdCapacidad" class="sdb-input" type="number" min="10" max="200" value="${game.capacidad_max||25}"></div>
          </div>
    ...
    <div class="panel-body no-pad" style="overflow-x:auto">
      <table id="tblRondas" style="width:100%">
        ...
        <tbody>${roundsData.map(r=>`<tr>
          ...
          <td><div class="gap2">
            <button class="btn btn-info btn-sm" onclick="verParticipantes('${r.id}','${gameNombre}','${r.numero}')" data-tip="Ver participantes"><i class="bi bi-people"></i></button>
            <button class="btn btn-ghost btn-sm" onclick="verComprobantes('${r.id}','${gameNombre}','${r.numero}')" data-tip="Ver comprobantes"><i class="bi bi-receipt"></i></button>
  ...
  const renderRondaCard = (r) => {
    ...
    return `<div class="ep-ronda-card">
      ...
      <div class="ep-ganadores-list">
        ${renderGanador(r.ganador_id,1)}${r.ganador2_id?renderGanador(r.ganador2_id,2):""}${r.ganador3_id?renderGanador(r.ganador3_id,3):""}
      </div>
    </div>`;
  };
  MC().innerHTML = `
