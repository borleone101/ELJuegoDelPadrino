...
// Poblar UI — verificar que los elementos existan
...
/* ════════════════════════════════════════════
   ROUTER — definido DESPUÉS de las funciones
   usando referencias tardías
════════════════════════════════════════════ */
  ...
  try {
    const [
      ...
      supabase.from("payments")
        .select("id,monto,estado,created_at,boletos_solicitados,profiles!payments_user_id_fkey(username),rounds(id,numero,games(nombre))")
        .eq("estado", "pendiente")
    ...
    MC().innerHTML = `
      ...
      <div class="stat-grid">
        ...
        <div class="sc"><div class="sc-bar b"></div><div class="sc-icon">❌</div>
          <div class="sc-val">${rechazadosPorMi ?? 0}</div><div class="sc-lbl">Rechacé</div></div>
      </div>
              ...
              : recientes.map(p => `
                <div class="act-row">
                  <div class="act-left">
                    ...
                    <div>
                      <div class="act-name">${p.profiles?.username ?? "—"}</div>
                  ...
                  <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.2rem">
                    <span class="act-amount">${fmtMoney(p.monto)}</span>
...
window.__aprobarDesdeCard = async (payId) => {
  ...
  try {
    const { data: pago, error } = await supabase.from("payments")
      .select("user_id, boletos_solicitados, round_id").eq("id", payId).single();
    ...
    if (partExist) {
      ...
      await supabase.from("participations").insert({
        round_id: pago.round_id, user_id: pago.user_id, boletos, resultado: "pendiente"
      });
    }
    Swal.close();
...
async function sorteos() {
  ...
  try {
    ...
    const gamesData = await Promise.all((games || []).map(async g => {
      ...
      let cupos = 0, compPend = 0;

      if (activeRound) {
        const [{ data: bRows }, { count: cp }] = await Promise.all([
      ...
      return { ...g, activeRound, totalRondas, cupos, compPend };
    }));
              ...
              <div class="sorteo-card">
                ...
                <div class="sorteo-card-mid">
                  ${ar ? `
                    ...
                    ${g.compPend > 0
                      ? `<div style="margin-top:.5rem;font-size:.77rem;color:var(--gold2)">
                           <i class="bi bi-exclamation-triangle"></i>
...
window.__verRondas = async (gameId, gameNombre) => {
  ...
  try {
    ...
    const roundsData = await Promise.all((rounds || []).map(async r => {
      const { data: bRows } = await supabase.from("participations")
        .select("boletos").eq("round_id", r.id);
      const cupos = (bRows || []).reduce((s, x) => s + (x.boletos || 1), 0);
      return { ...r, cupos };
    }));
...
window.__verParticipantes = async (roundId, gameNombre, num) => {
  ...
  try {
    const { data } = await supabase
      .from("participations")
      .select("id,boletos,resultado,created_at,profiles(username,email)")
      .eq("round_id", roundId);
    ...
    MC().innerHTML = `
      <div class="ph">
        <div class="ph-left">
          ...
          <div class="ph-sub">${gameNombre} · Ronda ${num} · ${data?.length ?? 0} jugadores · ${totalBoletos} boletos</div>
        </div>
        ${renderBackBtn("Volver a sorteos", sorteos)}
      </div>
      <div class="panel">
        ...
        <div class="panel-body" style="overflow-x:auto">
          <table id="tblPart" style="width:100%">
            <thead><tr><th>#</th><th>Usuario</th><th>Email</th><th>Boletos</th><th>Resultado</th><th>Inscripción</th></tr></thead>
            <tbody>
              ${(data || []).map((p, i) => `
                <tr>
    ...
    initDT("tblPart", { pageLength: 25, columnDefs: [{ orderable: false, targets: [0, 4] }] });
  } catch(e) {
                ...
                <tbody>
                  ${sorted.map(p => `
                    <tr>
                      ...
                      <td style="font-size:.82rem;color:var(--muted)">${p.metodo ?? "—"}</td>
...
window.__verComprobantes = async (roundId, gameNombre, num) => {
  loadingView();
  try {
    // SIN join a rounds — igual que admin.js, solo filtrar por round_id
    const { data, error } = await supabase
      .from("payments")
    ...
    const pendCount = sorted.filter(p => p.estado === "pendiente").length;
    window.__compMap = {};
    // Guardamos roundId del parámetro porque ya no tenemos join a rounds
    sorted.forEach(p => {
                ...
                <tbody>
                  ${sorted.map(p => `
                    <tr>
                      ...
                      <td>${p.estado === "pendiente"
                        ? `<div style="display:flex;gap:.3rem">
                             <button class="btn btn-success btn-sm" onclick="window.__aprobarPago('${p.id}','${roundId}','${gNomSafe}','${num}')"><i class="bi bi-check-lg"></i> OK</button>
...
window.__aprobarPago = async (id, roundId, gameNombre, num) => {
  const r = await confirm$("Aprobar pago", "¿El comprobante es válido y el monto es correcto?", "✅ Sí, aprobar");
  if (!r.isConfirmed) return;
  loading$("Aprobando...");

  try {
    const { data: pago, error } = await supabase.from("payments")
      .select("user_id, boletos_solicitados, round_id").eq("id", id).single();
    if (error || !pago) throw error || new Error("Pago no encontrado");
    ...
    Swal.close();
    toast(`✅ Aprobado · ${boletos} boleto${boletos !== 1 ? "s" : ""} confirmado${boletos !== 1 ? "s" : ""}`);
...
async function historial() {
  ...
  try {
    const { data: revisados } = await supabase
      .from("payments")
      .select("id,monto,estado,boletos_solicitados,created_at,profiles!payments_user_id_fkey(username),rounds(numero,games(nombre))")
        ...
        <div class="panel-body" style="overflow-x:auto">
          ${!revisados?.length
            ...
            : `<table id="tblHist" style="width:100%">
                <thead><tr><th>Usuario</th><th>Sorteo · Ronda</th><th>Boletos</th><th>Monto</th><th>Decisión</th><th>Fecha</th></tr></thead>
                <tbody>
                  ${revisados.map(p => `
                    <tr>
                      <td><strong>${p.profiles?.username ?? "—"}</strong></td>
    ...
    if (revisados?.length) initDT("tblHist", { order: [[5, "desc"]] });
  } catch(e) {
